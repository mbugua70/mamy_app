...
import { SummaryFormThree } from "../http/api";

const EyeClinics = () => {

  const { data, mutate, isError, error, isPending, isSuccess } = useMutation({
    mutationFn: SummaryFormThree,
    ...
    return (
      <>
        <AuthContentTwo onAuthenticate={submitHandler} isEyeClinicScreen={true} isPending={isPending} isSuccess={isSuccess}  isError={isError} />
      </>
  )
}
